...
class TransformPair:
    def __init__(self, horizontal_flip_prob=0.5, rotation_degrees=(0, 90, 180, 270)):
    ...
    def __call__(self, img, mask):
        ...
        if self.rotation_degrees:
            degree = random.choice(self.rotation_degrees)
...
def getClassWeights():
    ...
    for cls in unique_classes:
        if cls < n_class:
            class_counts[cls] += 1
    class_weights = n_samples / (n_class * class_counts)
    ...
    class_weights = torch.clamp(class_weights, min_weight, max_weight)

    class_weights = class_weights * (n_class / class_weights.sum())

    print("\nClass distributions and weights:")
    for cls in range(n_class):
        print(f"Class {cls}: {class_counts[cls]:.0f} images, weight: {class_weights[cls]:.4f}")
...
if FOUR_B:
    transform_pair = TransformPair(horizontal_flip_prob=0.5, rotation_degrees=(0, 90, 180, 270))

    class TrainTransform:
        def __call__(self, img):
            mask = img.copy() 
            img, mask = transform_pair(img, mask) 
    ...
    train_target_transform = MaskToTensor()
else:
...
def default_target_transform(mask):
    return MaskToTensor()(mask)


train_dataset = voc.VOC('train', transform=train_transform, target_transform=train_target_transform)


val_dataset = voc.VOC('val', transform=default_transform, target_transform=default_target_transform)
...
optimizer = torch.optim.Adam(fcn_model.parameters(), lr=0.005)
...
def plot_losses(train_losses, val_losses, best_epoch, early_stop_epoch):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("baseline Train and Validation Loss vs. Epochs")
    plt.legend()
    plt.grid(True)
    plt.savefig('XXXXXXXXXXXXXX_loss_plot.png')
    plt.close()

def train(): ######################################################################################### This Is Train
    best_iou_score = 0.0
    ...
    for epoch in range(epochs):
        ...
        print("Finish epoch {}, time elapsed {:.4f}, training loss: {:.4f}, validation loss: {:.4f}".format(
        ...
        # **Early Stopping**
        if not Early_stopped and patience_counter >= patience:
            early_stop_epoch = epoch - patience + 1
            print(f"Early stopping triggered at epoch {epoch + 1} (stopped training at epoch {early_stop_epoch})")
    ...
    if patience_counter < patience:
        early_stop_epoch = epochs
        print(f"No early stopping occurred. Training completed till epoch {early_stop_epoch}.")
