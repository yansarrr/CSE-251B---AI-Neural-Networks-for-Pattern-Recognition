...
def getClassWeights():
    """
    Calculate class weights using balanced mode with increased separation.
    Uses power scaling to create more distinction between common and rare classes.
    
    Returns:
        torch.Tensor: Tensor of class weights for loss function
    """
    # Initialize class counts
    class_counts = torch.zeros(n_class)
    
    print("Calculating class weights...")
    # Count occurrences of each class in training dataset
    n_samples = len(train_loader.dataset)  # Number of images in dataset
    ...
    for _, target in train_loader:
        ...
        # Count unique classes in each batch
        unique_classes = torch.unique(target)
        for cls in unique_classes:
            if cls < n_class:  # Ensure we only count valid classes
                class_counts[cls] += 1
    
    # Calculate initial balanced weights
    class_weights = n_samples / (n_class * class_counts)
    
    # Handle zero-frequency classes
    class_weights[class_counts == 0] = 0.0
    
    # Apply power scaling to create more separation
    power = 1.5  # Increase this value for more separation
    class_weights = torch.pow(class_weights, power)
    
    # Apply min/max thresholds with wider range
    min_weight = 0.05  # Decreased from 0.1
    max_weight = 20.0  # Increased from 10.0
    
    # Clip weights to [min_weight, max_weight]
    class_weights = torch.clamp(class_weights, min_weight, max_weight)
    
    # Normalize weights so their sum equals n_class
    class_weights = class_weights * (n_class / class_weights.sum())
    
    # Print class distributions and weights
...
# Update the criterion to use weighted loss
...
def plot_losses(train_losses, val_losses, best_epoch, early_stop_epoch):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Q4c Train and Validation Loss vs. Epochs")
    plt.legend()
    plt.grid(True)
